...
import { ensureUniqueSlug, normalizeSku, normalizeTagNames, productRelationInclude, resolveDueCollectionLaunches, serializeTaxonomyRelations, slugifyCatalogText, syncCollectionProducts, syncProductCollections, syncProductTags } from "../lib/catalog-taxonomy.mjs"
...
function serializeAdminCollection(collection) {
  return {
    ...
    createdAt: collection.createdAt ? collection.createdAt.toISOString() : undefined,
    updatedAt: collection.updatedAt ? collection.updatedAt.toISOString() : undefined,
    _count: collection._count || { products: Array.isArray(collection.products) ? collection.products.length : 0 },
    products: Array.isArray(collection.products)
      ? collection.products
          .slice()
          .sort((a, b) => (a.position || 0) - (b.position || 0))
          .map((entry) => serializeAdminProduct(entry.product))
      : [],
  }
}
...
const productSchema = z.object({
  ...
  personalizationTextEnabled: z.boolean().default(false),
  personalizationTextRequired: z.boolean().default(false),
  personalizationTextLabel: z.string().trim().max(160).optional().nullable(),
  personalizationTextMaxChars: z.number().int().min(1).max(200).default(50),
  personalizationImageEnabled: z.boolean().default(false),
  personalizationImageRequired: z.boolean().default(false),
  personalizationImageLabel: z.string().trim().max(160).optional().nullable(),
  personalizationImageInstructions: z.string().trim().max(1000).optional().nullable(),
...
const collectionSchema = z.object({
  ...
  promoText: z.string().trim().optional().nullable(),
  status: z.enum(["draft", "scheduled", "live", "archived"]).default("live"),
...
function parseCollectionLaunchAt(value) {
  if (!value) return null
  const date = new Date(value)
  if (Number.isNaN(date.getTime())) {
    throw new HttpError(400, "Data lancio collezione non valida")
  }
  return date
}
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.status(201).json(serializeAdminCollection(hydrated))
  })
)
  ...
  asyncHandler(async (req, res) => {
    ...
    const product = await prisma.product.create({
      data: {
        ...
        personalizationTextEnabled: Boolean(body.isCustomizable && body.personalizationTextEnabled),
        personalizationTextRequired: Boolean(body.isCustomizable && body.personalizationTextEnabled && body.personalizationTextRequired),
        personalizationTextLabel: body.isCustomizable && body.personalizationTextEnabled ? body.personalizationTextLabel?.trim() || null : null,
        personalizationTextMaxChars: Number(body.personalizationTextMaxChars || 50),
        personalizationImageEnabled: Boolean(body.isCustomizable && body.personalizationImageEnabled),
        personalizationImageRequired: Boolean(body.isCustomizable && body.personalizationImageEnabled && body.personalizationImageRequired),
        personalizationImageLabel: body.isCustomizable && body.personalizationImageEnabled ? body.personalizationImageLabel?.trim() || null : null,
        personalizationImageInstructions: body.isCustomizable && body.personalizationImageEnabled ? body.personalizationImageInstructions?.trim() || null : null,
  ...
  asyncHandler(async (req, res) => {
    ...
    const duplicated = await prisma.product.create({
      data: {
        ...
        personalizationTextEnabled: Boolean(product.personalizationTextEnabled),
        personalizationTextRequired: Boolean(product.personalizationTextRequired),
        personalizationTextLabel: product.personalizationTextLabel,
        personalizationTextMaxChars: Number(product.personalizationTextMaxChars || 50),
        personalizationImageEnabled: Boolean(product.personalizationImageEnabled),
        ...
        personalizationImageInstructions: product.personalizationImageInstructions,
        featured: false,
        stock: product.stock,
