...
test("getShippingMethodConfig returns centralized pricing and carrier data", () => {
  ...
  assert.equal(economy.description, "5 giorni lavorativi")
  assert.equal(premium.label, "Premium")
  assert.equal(premium.description, "3 giorni lavorativi")
  assert.equal(economy.cost, 700)
  assert.equal(premium.cost, 1000)
  assert.equal(formatShippingMethodSummary("premium"), "Premium — 3 giorni lavorativi")
})
