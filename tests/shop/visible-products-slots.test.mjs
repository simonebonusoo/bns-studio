...
test("admin products section uses simple homepage visibility controls instead of slot management", () => {
  ...
  assert.match(homeShop, /HOMEPAGE_TRUST_SECTION_KEY = "homepageTrustSection"/)
  assert.match(homeShop, /parseHomepageTrustSection/)
  assert.match(homeShop, /editingTrust/)
  assert.match(homeShop, /Label sezione Fiducia/)
  assert.match(homeShop, /Titolo card Fiducia/)
  assert.match(homeShop, /Aggiungi card/)
  assert.match(homeShop, /Elimina card/)
  assert.match(homeShop, /\/admin\/settings/)
