...
test("guest registration promo is mounted safely and uses the existing auth registration flow", () => {
  ...
  assert.match(popup, /Sblocca il 20% di sconto/)
  assert.match(popup, /Ottieni il 20%/)
...
test("first registration discount rule generates a unique owner-bound coupon", () => {
  ...
  assert.match(adminDiscounts, /<option value="buy_3_pay_2">3x2 \/ paghi 2 prendi 3<\/option>/)
  assert.match(adminDiscounts, /Ogni 3 articoli nel carrello, il meno costoso è gratuito\./)
  assert.match(adminDiscounts, /rule\.ruleType === "buy_3_pay_2"[\s\S]*"3x2"/)
  assert.match(adminDiscounts, /ogni 3 articoli, il meno costoso gratis/)
  ...
  assert.match(adminPage, /ruleForm\.ruleType === "buy_3_pay_2"/)
  assert.match(adminPage, /ruleForm\.ruleType === "buy_3_pay_2"[\s\S]*\? 3/)
  assert.match(adminPage, /ruleForm\.ruleType === "buy_3_pay_2"[\s\S]*\? "fixed"/)
  assert.match(adminPage, /ruleForm\.ruleType === "buy_3_pay_2"[\s\S]*\? 0/)
  assert.match(adminRoutes, /"first_registration"/)
  assert.match(adminRoutes, /"buy_3_pay_2"/)
  ...
  assert.match(pricing, /calculateBuy3Pay2Discount/)
  assert.match(pricing, /threeForTwoDiscounts/)
  ...
  assert.match(seed, /name: "3x2"/)
  assert.match(seed, /ruleType: "buy_3_pay_2"/)
